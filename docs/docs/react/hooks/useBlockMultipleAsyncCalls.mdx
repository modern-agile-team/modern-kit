...
위와 같은 한계점을 대응하고자 한다면 `useSingleRequest`를 사용할 수 있습니다.

@tanstack/react-query와 함께 사용 할 때와 같이 `상태 관리`가 필요하지 않다면 [`useSingleRequest`](/docs/react/hooks/useSingleRequest)를 사용하세요.
