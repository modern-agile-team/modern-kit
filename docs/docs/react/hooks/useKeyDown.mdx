...
`keyDownCallbackMap`의 key는 `KeyboardEvent.key`가 반환하는 값이 들어가야 합니다.
  - 💡 [Key values for keyboard events](https://developer.mozilla.org/en-US/docs/Web/API/UI_Events/Keyboard_event_key_values) 을 기반으로 자주 사용되는 `KeyboardEvent.key`들은 `타입 추론`을 지원합니다. 타입 추론되지 않은 `KeyboardEvent.key`는 직접 입력하셔야 됩니다.
