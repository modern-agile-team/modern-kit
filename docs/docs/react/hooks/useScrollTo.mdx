...
훅이 반환하는 `ref`를 특정 엘리먼트에 할당 후 함께 반환하는 `scrollTo`를 통해 해당 엘리먼트를 대상으로 `scrollTo`를 호출하는 커스텀 훅입니다. 

- `ref`를 할당하지 않으면 기본적으로 `window`에서 동작합니다.

`useScrollTo` 인자인 `autoScrollOptions`를 할당하면 마운트 시 자동으로 `scrollTo`가 호출됩니다.
...
const Example = () => {
  const { ref, scrollTo } = useScrollTo<HTMLDivElement>({
    ...
    left: 0,
    top: 400,
  }); // autoScrollOptions을 인자로 넘겨야 마운트 시 자동으로 scrollTo가 동작합니다.
