...
controlled mode에서는 value는 외부에서 제어되며, 해당 훅이 반환하는 setValue 함수를 통해 value를 변경할 수 없습니다.

uncontrolled mode에서는 defaultValue가 초기값으로 사용되며, 해당 훅이 반환하는 setValue 함수를 통해 value를 변경할 수 있습니다.
...
const ControlledExample = () => {
  ...
  const [value, setValue] = useControllableState({ value: externalValue });
