...
| --- | --- | --- |
| `copiedData` | `string \| Blob \| null` | 최근 클립보드에 복사된 데이터입니다. |
| `readData` | `string \| ClipboardItems \| null` | 클립보드에서 읽어온 데이터입니다. |
| `copyText` | `(value: string) => Promise<boolean>` | 주어진 텍스트를 클립보드에 복사하는 함수입니다. 성공 여부를 반환합니다. |
| `copyImage` | `(src: string, options?: { toText: boolean }) => Promise<boolean>` | 주어진 이미지 URL을 클립보드에 복사하는 함수입니다. 성공 여부를 반환합니다. |
| `readText` | `() => Promise<boolean>` | 클립보드에 저장된 텍스트 데이터를 읽어오는 함수입니다. 성공 여부를 반환합니다. |
| `readContents` | `() => Promise<boolean>` | 클립보드에 저장된 텍스트, HTML, 이미지 등 다양한 유형의 컨텐츠를 읽어오는 함수입니다. 성공 여부를 반환합니다. |
