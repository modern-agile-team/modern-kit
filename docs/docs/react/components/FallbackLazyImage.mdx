...
참고로, style props을 허용하지 않습니다. `css-in-js 상속` 또는 `외부 css을 통해 스타일을 제어`해주세요.
...
### Styling: css
- FallbackLazyImage 컴포넌트 내 Image Wrapper는 기본적으로 `lazy-image-wrapper` 클래스를 갖습니다. 해당 클래스를 활용하여 스타일을 제어 할 수 있습니다.
  - 물론, 직접 클래스를 넘겨 줄 수도 있습니다.
```css
/* 기본 클래스 */
.lazy-image-wrapper {
  border-radius: 12px;
