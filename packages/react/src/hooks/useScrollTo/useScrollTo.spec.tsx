...
describe('useScrollTo', () => {
  it('인자가 제공되지 않았을 때 기본 위치로 스크롤되어야 합니다', async () => {
  ...
  it('div 요소에서 scrollToPosition이 올바른 옵션과 함께 호출되어야 합니다', async () => {
  ...
  it('window에서 scrollToElement가 올바른 옵션과 함께 호출되어야 합니다', async () => {
    const { result } = renderHook(() => useScrollTo<HTMLDivElement>());
  ...
  it('요소로 스크롤할 때 container 요소에서 scrollToElement가 올바른 옵션과 함께 호출되어야 합니다', async () => {
    const { result } = renderHook(() => useScrollTo<HTMLDivElement>());
