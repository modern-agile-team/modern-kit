...
describe('useArrayState', () => {
  ...
  it('setArray 직접 호출로 배열을 변경해야 합니다.', async () => {
    ...
    setArray([4]);
    await waitFor(() => expect(result.current[0]).toEqual([4]));
  });
  ...
  it('setArray.push 메서드로 아이템을 추가해야 합니다.', async () => {
    ...
    push(4, 3);
    await waitFor(() => expect(result.current[0]).toEqual([4, 3]));
  });
  ...
  it('setArray.unshift 메서드로 아이템을 추가해야 합니다.', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toEqual([1, 2, 3, 4]));
  });
  ...
  it('setArray.pop 메서드로 마지막 아이템을 제거해야 합니다.', async () => {
    ...
    setArray.pop();
    await waitFor(() => expect(result.current[0]).toEqual([1, 2]));
  });
  ...
  it('setArray.shift 메서드로 첫번째 아이템을 제거해야 합니다.', async () => {
    ...
    setArray.shift();
    await waitFor(() => expect(result.current[0]).toEqual([2, 3]));
  });
  ...
  it('setArray.splice 메서드로 특정 인덱스의 아이템을 제거하고 추가해야 합니다.', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toEqual([1, 4, 5, 3]));
  });
  ...
  it('setArray.removeAt 메서드로 특정 인덱스의 아이템을 제거해야 합니다.', async () => {
    ...
    setArray.removeAt(1);
    await waitFor(() => expect(result.current[0]).toEqual([1, 3]));
  });
  ...
  it('setArray.updateAt 메서드로 특정 인덱스의 아이템을 변경해야 합니다.', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toEqual([1, 5, 3]));
  });
  ...
  it('setArray.updateBy 메서드는 iteratee 함수를 통해 배열의 모든 아이템을 변경해야 합니다.', async () => {
    ...
    await waitFor(() => expect(result.current[0]).toEqual([1, 3, 4]));
  });
