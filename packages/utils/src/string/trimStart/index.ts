...
/**
 * @description 문자열에서 `지정된 문자` 또는 `문자들의 조합`을 문자열 시작 부분에서 찾아 제거합니다.
 *
 * 문자열을 각 문자로 나누고 문자열 시작 부분에서 이를 찾아 제거합니다.
 * - ex: `"+-*" -> ['+', '-', '*']`
 *
 * @param {string} str - 전체 문자열입니다.
 * @param {string} chars - 문자열 앞부분에서 제거하고 싶은 문자열입니다.
 * @returns {string} - str에서 앞에있는 지정된 문자열을 제거한 문자열입니다.
 *
 * @example
 ...
 *
 * 배열 내 문자열을 각 문자로 나누고 문자열 시작 부분에서 이를 찾아 제거합니다.
 * - ex: `['+*', '-'] -> ['+', '*', '-']`
 *
 * @param {string} str - 전체 문자열입니다.
 * @param {string[]} chars - 문자열 앞부분에서 제거하고 싶은 문자열 배열입니다.
 * @returns {string} - str에서 앞에있는 공백 또는 지정된 chars를 제거한 문자열입니다.
 *
 * @example
 * trimStart('-_-abc', ['-', '_']); // 'abc-_-', 배열 내 문자열 요소인 ('-', '_')를 모두 찾아 제거
 * trimStart('+-*abc', ['+*', '-']); // 'abc', 배열 내 문자열 요소는 개별 문자로 나누고('+', '*', '-')이를 모두 찾아 제거
 */
export function trimStart(str: string, chars: string[]): string;
 ...
 *
 * @example
 * trimStart('-_-abc', ['-', '_']); // 'abc', 선행에서 배열 요소인 '-'와 '_'를 모두 제거
 * trimStart('+-*abc', ['+*', '-']); // 'abc', 배열 내 문자열 요소는 개별 문자로 나누고('+', '*', '-') 이를 모두 찾아 제거
 */
