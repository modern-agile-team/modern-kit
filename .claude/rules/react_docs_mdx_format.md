...
### 함수 오버로딩 (구현 코드에 오버로드가 있을 때)

- 훅/함수가 함수 오버로딩(여러 개의 시그니처)으로 작성되어 있으면, `## Interface`에도 **모든 오버로드 시그니처를 그대로 나열**합니다. 단일 시그니처로 뭉뚱그리지 않습니다.
- 각 시그니처가 어떤 호출 케이스에 해당하는지 짧은 주석으로 구분합니다. (예: `// 1. 옵션 없이 호출`)
- 오버로드마다 인자 타입(옵션 객체 등)이 다르면 해당 타입 정의도 Interface에 함께 표기합니다.
- `## Usage` 및 인터랙티브 예제(`### Example`)는 각 오버로드에 대응하는 케이스를 분리해 작성합니다.
- `### Options` / `### Parameters` 테이블에는 각 속성이 어떤 오버로드(옵션 타입)에 속하는지 명시합니다.
