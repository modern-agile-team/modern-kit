...
### 함수 오버로딩 / 조건부 타입 분기 (구현 코드에 분기가 있을 때)

- 타입에 딸린 헬퍼 함수가 함수 오버로딩(여러 개의 시그니처)으로 작성되어 있으면, `## Interface`에도 **모든 오버로드 시그니처를 그대로 나열**합니다. 단일 시그니처로 뭉뚱그리지 않습니다.
- 타입 자체가 조건부 타입(`extends ? :`)이나 입력에 따라 결과가 갈리는 분기 구조라면, 분기 조건과 각 분기의 결과 타입을 `## Interface`/설명에 명확히 드러냅니다.
- 각 시그니처/분기가 어떤 입력 케이스에 해당하는지 짧은 주석으로 구분합니다. (예: `// 1. 배열을 전달한 경우`)
- `## Usage`는 각 오버로드/분기에 대응하는 케이스를 `### 케이스 제목` 소제목으로 분리하고, 결과 타입을 주석으로 표기합니다.
